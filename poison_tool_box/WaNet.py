...
class poison_generator():
    ...
    def __init__(self, img_size, dataset, poison_rate, cover_rate, path, identity_grid, noise_grid, delta, s=0.5, k=4, grid_rescale=1, target_class=0):
    ...
    def generate_poisoned_training_set(self):
        ...
        for i in range(self.num_img):
            ...
            if ct < num_cover and cover_indices[ct] == i:
                ...
                img_temp = F.grid_sample(img.unsqueeze(0), grid_temps2, align_corners=True)[0]
            ...
            if pt < num_poison and poison_indices[pt] == i:
                ...
                img_temp = F.grid_sample(img.unsqueeze(0), grid_temps, align_corners=True)[0]
...
class poison_transform():
    ...
    def __init__(self, img_size, normalizer, denormalizer, identity_grid, noise_grid, delta, s=0.5, k=4, grid_rescale=1, target_class=0):
    ...
    def transform(self, data, labels):
        grid_temps = (self.identity_grid + self.s * self.noise_grid / self.img_size) * self.grid_rescale
        grid_temps = torch.clamp(grid_temps, -1, 1)
        # grid_temps = grid_temps.unsqueeze(0)
        # grid_temps = grid_temps.repeat(data.shape[0], 1, 1, 1)

        data, labels = data.clone(), labels.clone()
        # data = self.denormalizer(data)

        if data.dim() == 3:
            data_temp = F.grid_sample(data.unsqueeze(0), grid_temps, align_corners=True)[0]
        else:
            data_temp = F.grid_sample(data, grid_temps.repeat(data.shape[0], 1, 1, 1), align_corners=True)[0]
