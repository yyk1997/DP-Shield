'''codes used to construct backbone (imagenette)
'''
import argparse
import os, sys
import time
...
model_folder_path = os.path.join(supervisor.get_poison_set_dir(args), f'noise_std={args.noise_std}')
...
if args.dataset != 'ember' and args.dataset != 'imagenet' and args.dataset != 'imagenette':
    ...
    if os.path.exists(denoised_poisoned_set_path):
        ...
        denoised_poisoned_set.transforms = data_transform if args.no_aug else data_transform_aug
    else:
        # 对数据集中加入噪声
        ...
        denoised_poisoned_set = tools.DenoisedPoisonedDataset(noisy_poisoned_set, ddpm, closest_step= 0,
                                                              scheduler=scheduler,
    ...
    poisoned_set = imagenet.imagenet_dataset_with_noise(directory=train_set_dir, poison_directory=poisoned_set_img_dir,
                                                        ...
                                                        target_class=imagenet.target_class,
    ...
    poisoned_set = imagenet.DenoisedPoisonedDataset(original_dataset=poisoned_set, ddpm=ddpm, closest_step=0,
                                                    scheduler=scheduler, data_transform= data_transform_aug)
...
for epoch in range(1, epochs + 1):  # train backdoored base model
    ...
    if epoch % 100 == 0:
        ...
        torch.save(model.state_dict(), os.path.join(model_folder_path, output_name))
