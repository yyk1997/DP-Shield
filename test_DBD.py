...
name = './DPC_test/poisoned_train_set/cifar10/badnet_0.005_10000_poison_seed=0/noise_std=0'
...
if args.dataset != 'ember' and args.dataset != 'imagenet':
    ...
    denoised_poisoned_test_set_path = os.path.join(folder_path,
                                                   f"denoised_poisoned_test_set_{args.delta_test}_without_noise.pt")
...
criterion = SCELoss(num_classes = 10).cuda()
epochs = 10
...
warmup_criterion = SCELoss(num_classes = 10, reduction= "none").cuda()
...
for epoch in range(10,200):
    ...
    poison_train_result = mixmatch_train(
        model, xloader, uloader, semi_criterion, optimizer, epoch, train_iteration = 1024, temperature = 0.5,alpha = 0.75,num_classes = 10
    )
        ...
        if True:
            if epoch % 5 == 0:
                if args.dataset == 'imagenet':
                    ...
                    tools.test_poisoned(dataset_name=args.dataset, model=model, test_loader=poisoned_test_set_loader,
                                        poison_test=True,
                                        poison_transform=None, num_classes=num_classes,
