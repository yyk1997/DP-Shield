'''codes used to construct smoothed classifier (gtsrb)
'''
import argparse
import os, sys
import time
...
# 数据集和后门攻击相关
args.dataset = "gtsrb"
folder_path = './poisoned_train_set/gtsrb/denoised_test_set'
...
bagging = int(args.bagging * len(denoised_poisoned_set))
...
for _ in range(0, args.N_m):
    ...
    if bagging != 0:
        ...
        all_indices = [i for i in range(len(denoised_poisoned_set))]

        sampled_indices = random.choices(all_indices, k=bagging)
