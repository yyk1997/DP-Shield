'''codes used to evalate the perturbation on test sample (cifar10)
'''
import argparse
import math
import os, sys
...
# cert_acc_all_poison_1.append(np.logical_and(certified_bound_all_poison_1, is_acc_1, pa > pb).sum() / (pa > pb).sum())
...
# # proba
# # np.logical_and用于逐元素的对两个数组执行“与”操作，cert_acc表示的是总鲁棒半径大于r且分类准确的概率
# cert_acc_all_poison_2.append(np.logical_and(certified_bound_all_poison_2, is_acc_2, pa > pb).sum() / (pa > pb).sum())
# # 条件认证准确率，表示在所有认证半径大于r的样本中，有多少样本的分类是正确的
