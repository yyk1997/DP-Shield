...
def get_poison_set_dir(args):
    ...
    if args.poison_type == 'blend' or args.poison_type == 'basic' or args.poison_type == 'clean_label':
        blend_alpha = '%.3f' % args.alpha
        poison_set_dir = 'poisoned_train_set/%s/%s_%s_alpha=%s_trigger=%s_%s' % (args.dataset, args.poison_type, ratio, blend_alpha, args.trigger, args.delta)
    elif args.poison_type == 'adaptive' or args.poison_type == 'adaptive_blend':
        blend_alpha = '%.3f' % args.alpha
        cover_rate = '%.3f' % args.cover_rate
        ...
        poison_set_dir = 'poisoned_train_set/%s/%s_%s_%s' % (args.dataset, args.poison_type, ratio, args.delta)
...
def get_poison_transform(poison_type, dataset_name, target_class, source_class=1, cover_classes=[5,7],
    ...
    if poison_type in ['badnet', 'blend', 'clean_label',
        ...
        if poison_type == 'badnet':
            ...
            poison_transform = badnet.poison_transform(img_size=img_size, trigger_mark=trigger, trigger_mask=trigger_mask, target_class=target_class, delta= args.delta_test)

        elif poison_type == 'trojan':
            from poison_tool_box import trojan
            poison_transform = trojan.poison_transform(img_size=img_size, trigger_mark=trigger, trigger_mask=trigger_mask, target_class=target_class, delta= args.delta_test)
            ...
            poison_transform = blend.poison_transform(img_size=img_size, trigger=trigger,
                                                      target_class=target_class, alpha=alpha, delta= args.delta_test)
            ...
            poison_transform = clean_label.poison_transform(img_size=img_size, trigger_mark=trigger, trigger_mask=trigger_mask,
                                                            target_class=target_class, delta = args.delta_test)
            ...
            poison_transform = WaNet.poison_transform(img_size=img_size, denormalizer=denormalizer, identity_grid=identity_grid, noise_grid=noise_grid, s=s, k=k, grid_rescale=grid_rescale, normalizer=normalizer, target_class=target_class, delta = args.delta_test)
            ...
            poison_transform = adaptive_patch.poison_transform(img_size=img_size, test_trigger_names=config.adaptive_patch_test_trigger_names[args.dataset], test_alphas=config.adaptive_patch_test_trigger_alphas[args.dataset], target_class=target_class, denormalizer=denormalizer, normalizer=normalizer,delta = args.delta_test)
            ...
            poison_transform = SIG.poison_transform(img_size=img_size, denormalizer=denormalizer, normalizer=normalizer,
                                                    target_class=target_class, delta=30/255, f=6, has_normalized= False, delta_attack = args.delta_test)
        ...
        poison_transform = dynamic.poison_transform(ckpt_path=ckpt_path, channel_init=channel_init, steps=steps,
                                                    ...
                                                    denormalizer=denormalizer,target_class=target_class, delta= args.delta_test,
        ...
        poison_transform = ISSBA.poison_transform(ckpt_path=ckpt_path, secret=secret, normalizer=normalizer, denormalizer=denormalizer,
                                                   enc_in_channel=input_channel, enc_height=img_size, enc_width=img_size, target_class=target_class, delta = args.delta_test)
        return poison_transform
