'''codes used to construct smoothed classifier (cifar10) without pretrain
'''
import argparse
import os, sys
import time
...
model_folder_path = os.path.join(supervisor.get_poison_set_dir(args), f'noise_std={args.noise_std}/DP_({MAX_GRAD_NORM}_{Noise_Multi}_bagging_{args.bagging}_no_pretrain)')
...
for _ in range(0, args.N_m):
            ...
            if True:
                if epoch % 3 == 0:
                    if args.dataset == 'imagenet':
                        ...
                        torch.save(model.projection.state_dict(), supervisor.get_model_dir(args))
                    else:
                        ...
                        torch.save(model.state_dict() if hasattr(model, "module") else model.state_dict(), supervisor.get_poison_set_dir(args) + f'/noise_std={args.noise_std}/DP_({MAX_GRAD_NORM}_{Noise_Multi}_bagging_{args.bagging}_no_pretrain)/{_}.pt')
    ...
    if args.dataset != 'ember':
        ...
        # epsilon_file_path = supervisor.get_poison_set_dir(
        #     args) + f"/noise_std={args.noise_std}/DP_({MAX_GRAD_NORM}_{Noise_Multi}_bagging_{args.bagging}_no_pretrain)/DP_Norm_epsilon_list.json"
        #
        # # 将列表保存到文件
        # import json
        #
    ...
    torch.save(privacy_state, supervisor.get_poison_set_dir(
            args) + f"/noise_std={args.noise_std}/DP_({MAX_GRAD_NORM}_{Noise_Multi}_bagging_{args.bagging}_no_pretrain)/privacy_engine_state.pth")
