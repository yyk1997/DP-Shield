'''codes used to evalate the certified robustness for the number of poisoned samples in training dataset (cifar10) input poisoned sample
'''
import argparse
import math
import os, sys
...
privacy_state_path = supervisor.get_poison_set_dir(args) + f"/noise_std={root_noise_std}/DP_({MAX_GRAD_NORM}_{Noise_Multi}_bagging_{args.bagging})/privacy_engine_state.pth"
...
pa_exp_1, pb_exp_1, is_acc_1, pa_exp_2, pb_exp_2, is_acc_2  = certificate_over_dataset(model=model, num_classes=num_classes,
                                                      dataloader=denoised_poisoned_test_set_loader, args=args, N_m=args.N_m)
