'''codes used to construct smoothed classifier (imagenette)
'''
import argparse
import os, sys
import time
...
folder_path = './poisoned_train_set/imagenette/denoised_test_set'
args.poison_type = "blend"
args.poison_rate = 0.010
...
# pretrain_name = './poisoned_train_set/imagenette/none_0.000_10000_poison_seed=0/noise_std=0/pretrain_model_200_epochs.pth'
...
if args.dataset != 'ember' and args.dataset != 'imagenet' and args.dataset != 'imagenette':
    ...
    root_dir = train_set_dir = os.path.join(data_dir, 'imagenette/imagenette2')
    train_set_dir = os.path.join(root_dir, 'train')
    test_set_dir = os.path.join(root_dir, 'val')

    from utils import imagenet

    poisoned_set = imagenet.imagenet_dataset_with_noise(directory=train_set_dir, poison_directory=poisoned_set_img_dir,
                                             poison_indices=poison_indices, target_class=imagenet.target_class,
                                             num_classes=10, noise_std= 0, closest_value= 1)
    poisoned_set = imagenet.DenoisedPoisonedDataset(original_dataset=poisoned_set, ddpm=ddpm, closest_step= 0,
                                     scheduler=scheduler)
...
if args.dataset != 'ember' and args.dataset != 'imagenet' and args.dataset != 'imagenette':
    ...
    denoised_test_set_path = os.path.join(folder_path, f"test_set_{args.noise_std}.pt")
    if os.path.exists(denoised_test_set_path):
        test_set = torch.load(denoised_test_set_path)
    else:
        noisy_test_set = imagenet.imagenet_dataset_with_noise(directory=test_set_dir, shift=False, aug=False,
                                                              label_file=None, num_classes=10, noise_std=args.noise_std,
                                                              closest_value= closest_value)

        test_set = imagenet.DenoisedPoisonedDataset(original_dataset=noisy_test_set, ddpm=ddpm,
                                                    closest_step= closest_index, scheduler=scheduler, aug=False)
        torch.save(test_set, denoised_test_set_path)

    # poison_transform = imagenet.get_poison_transform_for_imagenet(args.poison_type)

    # denoised_test_set_backdoor_path = os.path.join(folder_path, f"test_set_backdoor_{args.delta_test}_without_denoised.pt")
    # if os.path.exists(denoised_test_set_backdoor_path):
    #     test_set_backdoor = torch.load(denoised_test_set_backdoor_path)
    # else:
    #     noisy_test_set_backdoor = imagenet.imagenet_dataset_with_noise(directory=test_set_dir, shift=False, aug=False,
    #                                                                    label_file=None, num_classes=10,
    #                                                                    poison_transform=poison_transform,
    #                                                                    delta=args.delta_test,
    #                                                                    noise_std=args.noise_std,
    #                                                                    closest_value= 1)
    #
    #     test_set_backdoor = imagenet.DenoisedPoisonedDataset(original_dataset=noisy_test_set_backdoor, ddpm=ddpm,
    #                                                          closest_step= 0, scheduler=scheduler, aug=False)
    #     torch.save(test_set_backdoor, denoised_test_set_backdoor_path)
...
for _ in range(0, args.N_m):
    ...
    if args.bagging != 0:
        ...
        all_indices = [i for i in range(9469)]
        ...
        subset_poisoned_set = torch.utils.data.Subset(poisoned_set, sampled_indices)

        poisoned_set_loader = DataLoader(subset_poisoned_set, batch_size=batch_size, shuffle=True,
        ...
        if args.dataset != 'ember':
            if True:
                if epoch % 5 == 0:
                    ...
                    torch.save(model.projection_head.state_dict(), supervisor.get_poison_set_dir(args)+ f'/noise_std={args.noise_std}/DP_({MAX_GRAD_NORM}_{Noise_Multi}_bagging_{args.bagging})/{_}.pt')
    ...
    if args.dataset != 'ember':
        torch.save(
            model.projection_head.state_dict() if hasattr(model, "module") else model.projection_head.state_dict(),
            supervisor.get_poison_set_dir(
                args) + f'/noise_std={args.noise_std}/DP_({MAX_GRAD_NORM}_{Noise_Multi}_bagging_{args.bagging})/{_}.pt')
