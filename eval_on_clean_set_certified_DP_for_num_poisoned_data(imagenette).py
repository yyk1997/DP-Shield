'''codes used to evalate the certified robustness for the number of poisoned samples in training dataset (imagnette)
'''
import argparse
import math
import os, sys
...
if args.dataset == 'imagenette':
    ...
    denoised_test_set_path = os.path.join(test_folder_path, f"test_set_{args.noise_std}.pt")
...
def compute_radius_in_differential_privay(privacy_engine, bagging_sample_rate, pa, pb):
    ...
    # for i in tqdm(range(0, len(pa_exp))):
    #     low = 0
    #     high = 50000 * args.poison_rate
    #     if check_condition_DP(privacy_engine, bagging_sample_rate, pa_exp[i], pb_exp[i], high):
    #         result.append(high)
    #     elif check_condition_DP(privacy_engine, bagging_sample_rate, pa_exp[i], pb_exp[i], 1):
    #         result.append(1)
    #     else:
    #         result.append(0)
    #
    #     # while low <= high:
    #     #     mid = (low + high)//2
    #     #     if check_condition_DP(privacy_engine, bagging_sample_rate, pa_exp[i], pb_exp[i], mid):
    #     #         low = mid + 1
    #     #     else:
    #     #         high = mid - 1
    #     # result.append(high)
...
def check_condition_DP(privacy_engine, bagging_sample_rate, pa_exp, pb_exp, radius):
    ...
    iters = [entry[2] for entry in history]
    iters_new = int(iters[0] * 4 * 0.6)
    total_epsilon = 0
    sum_value = 0
    Delta_total = 0

    for i in range(1, r + 1):
        sam_rate = 1 - ((1 - q[0] / 4) ** i)

        updated_history = [
            (entry[0], sam_rate, iters_new)
            for entry in accountant_state["history"]
        ]
    ...
    # total_epsilon = 1e-8 + np.log(1 + (bagging_sample_rate ** radius) * (np.exp(epsilon) - 1))
...
print(f"Certified Radius:", {9469 * args.poison_rate})
